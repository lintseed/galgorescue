...
<body<?php if ( is_front_page() || is_home() ) echo ' id="home"'; ?> <?php if ( function_exists('body_class') ) body_class(); ?>>
		...
		<div id="header">
			<div class="container clearfix">
				...
				<a href="<?php bloginfo('url'); ?>">
					...
					<img src="<?php echo $logo; ?>" alt="Nova Logo" id="logo"/>
				</a>
				...
				if ($primaryNav == '') { ?>
					<ul id="<?php echo $menuID; ?>" class="<?php echo $menuClass; ?>">
						<?php if (get_option('nova_home_link') == 'on') { ?>
							<li <?php if (is_home()) echo('class="current_page_item"') ?>><a href="<?php bloginfo('url'); ?>"><?php _e('Home','Nova') ?></a></li>
						<?php }; ?>
				...
				<div id="search-form">
					...
					<form method="get" id="searchform" action="<?php bloginfo('url'); ?>">
						<input type="text" value="<?php _e('search this site...','Nova'); ?>" name="s" id="searchinput" />
					</form>
				</div> <!-- end #search-form -->
			</div> <!-- end .container -->
		</div> <!-- end #header -->
		
		<?php if ( (is_front_page() || is_home()) && get_option('nova_featured') == 'on' ) include(TEMPLATEPATH . '/includes/featured.php');?>
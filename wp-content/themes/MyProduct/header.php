...
<body<?php if (is_front_page() || is_home()) echo(' id="home"'); ?>>
	<div id="header-wrapper">
		<div id="header">

			<div class="container">
				<a href="<?php bloginfo('url'); ?>"><?php $logo = (get_option('myproduct_logo') <> '') ? get_option('myproduct_logo') : get_bloginfo('template_directory').'/images/logo.png'; ?>
					...
					<div id="topmenu">
						...
						if ($primaryNav == '') { ?>
							<ul class="<?php echo $menuClass; ?>">
								<?php if (get_option('myproduct_home_link') == 'on') { ?>
									<li <?php if (is_front_page() || is_home()) echo('class="current_page_item"') ?>><a href="<?php bloginfo('url'); ?>" id="homelink"><span><?php _e('Home','MyProduct'); ?></span></a></li>
								<?php }; ?>
						...
						<div id="search-form">
							<form method="get" id="searchform1" action="<?php bloginfo('url'); ?>/">
								<input type="text" value="<?php _e('search this site...','MyProduct'); ?>" name="s" id="searchinput" />
							</form>
						</div>				
						End Searchbox -->													
					</div> <!-- end #topmenu -->
				...
				<?php if (is_front_page() || is_home()) { ?>
					...
					<?php query_posts('page_id=' . get_pageId(html_entity_decode(get_option('myproduct_welcome_page'))) ); 
						if(have_posts()) : while (have_posts()) : the_post(); ?>
							<div id="top-box">
								...
								<?php global $more;   
                                $more = 0; the_content(''); ?>
                                <?php $button1 = get_post_meta($post->ID, 'Button', true);
									  $button1Url = get_post_meta($post->ID, 'Buttonurl', true);
									  $button2 = get_post_meta($post->ID, 'Button2', true);
								      $button2Url = get_post_meta($post->ID, 'Button2url', true); ?>
								
								<?php if ($button1 <> '') { ?>
									<a href="<?php echo($button1Url); ?>" class="featured-button"><span><?php echo($button1); ?></span></a>
								<?php }; ?>
								<?php if ($button2 <> '') { ?>
									<a href="<?php echo($button2Url); ?>" class="featured-button"><span><?php echo($button2); ?></span></a>
								<?php }; ?>
							</div> <!-- end #top-box -->
					...
					<div id="image_slideshow">
						<div id="images">
							<?php $productsNumber = get_option('myproduct_product_images_number');
							for ($i = 1; $i <= $productsNumber; $i++) { ?>
								<img src="<?php echo(get_option('myproduct_product_image_'.$i)); ?>" alt="" />
							<?php }; ?>
						</div> <!-- end #images -->
				...
				<?php } else include(TEMPLATEPATH . '/includes/pagetop.php'); ?>

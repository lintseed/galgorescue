...
function vr_widget_init() {
	...
	wp_register_sidebar_widget('vertical-response', 'Vertical Response', 'widget_vr', array('description' => 'Add a Vertical Response signup form.'));
	wp_register_widget_control('vertical-response', 'Vertical Response', 'widget_vr_options', array('width' => 500));
}
...
function widget_vr_options() {
	...
	if (!is_array($options)) {
		$options = array(
			'title' => __('Vertical Response', 'vertical-response'), 
			'code' => '', 
			'button' => 'Subscribe', 
			'wrap' => 'p', 
			'showname' => 'full', 
...
?>
	...
	<div style="-webkit-border-radius: 3px; margin: 5px 0; color: #333; border-radius: 3px; border-width: 1px; border-style: solid; background-color: lightYellow; border-color: #E6DB55; padding:10px;">
		<p style="padding:0; margin:0; line-height:1.3;">A Vertical Response account is required to use this plugin. <strong><a href="http://katz.si/vr" target="_blank">Get a free trial now</a></strong>.</p>
	</div>
